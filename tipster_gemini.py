...
# Groq API keys (PRIMARIO — 6.000 req/giorno, 30 req/min, molto più generoso di Gemini)
GROQ_API_KEYS = [k for k in [
    os.environ.get("GROQ_API_KEY", ""),
    os.environ.get("GROQ_API_KEY_2", ""),
] if k]
# Gemini API keys (FALLBACK se Groq è esaurito o down)
...
GEMINI_API_KEY = GEMINI_API_KEYS[0] if GEMINI_API_KEYS else ""  # backward compat per genera_immagine
...
def groq_decide(prompt):
    """Chiama Groq (llama-3.3-70b) per decisione schedina — primario."""
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                try:
                    ...
                    print(f"Groq OK — pubblica={decision.get('pubblica')}")
                    return decision
...
def ai_decide(games, fascia):
    """Entry point AI: prova Groq (primario), poi Gemini (fallback)."""
    # Costruiamo il prompt qui per passarlo a Groq o Gemini
...
{{"pubblica": true, "motivo_no": null, "tipo": "combo", "picks": [{{"home": "squadra", "away": "squadra", "competition": "nome", "sport_key": "key", "time": "HH:MM", "sign": "1", "odd": 1.5}}], "total_odd": 8.5, "vincita": 85.0, "signs_string": "1 - X - 2", "ragionamento": "spiegazione"}}"""
    ...
    # 1) Prova Groq
    if GROQ_API_KEYS:
        print("Provo Groq (primario)...")
        result = groq_decide(prompt)
        if result is not None:
            return result
        print("Groq fallito — fallback su Gemini")
    else:
        print("Nessuna GROQ key configurata — uso Gemini direttamente")

    # 2) Fallback Gemini (passa solo il prompt, gemini_decide lo ricostruisce internamente)
    return gemini_decide(games, fascia)
...
if __name__ == "__main__":
    ...
    if not games:
        ...
        sys.exit(0)
    print("\nAI sta decidendo (Groq → Gemini fallback)...")
    decision = ai_decide(games, FASCIA)
    if not decision:
        print("Errore AI (Groq + Gemini). Stop.")
        sys.exit(1)
